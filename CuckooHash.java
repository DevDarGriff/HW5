...
*/
         ...
         Bucket<K, V> buck = new Bucket<K, V>(key, value);                           //buck variable to hold key, value pair
      
         if (table[pos] != null && table[pos].getBucKey().equals(key)) {           //pos1 is not null && pos1 keys match
            return; 
        }

        //int evictions = 0;

        //key is not here - enter for loop to check bucket is null or not
         for (int i = 0; i < size(); i++) {                       //loop through entirity of Cuckoo Hash - CAPACITY, size(), table.length
            if (table[pos] == null) {                               //if table pos is null
                table[pos] = buck;                                  //set table pos to bucket
                return;
            } else {        //buck is not empty
                Bucket<K, V> kickedOut = table[pos];                //set kickedOut = 
                table[pos] = buck;
                buck = kickedOut;
...
*/
                ...
                if (hash1(buck.getBucKey()) == pos) {               //if bucket key == pos
                    pos = hash2(buck.getBucKey());                  //set pos = bucket key's 2nd hash
                } else {
                    pos = hash1(buck.getBucKey());                  //set pos = bucket key's 1st hash
                }
            }
         }
